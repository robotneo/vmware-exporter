...
> **本文写作后的进展（2026-09-03 更新）**：上面这条结论已部分过时。
> **vSAN 现已采集**（`-collector.vsan` 与 `-collector.vsan.perf`，
> 均默认禁用），**ResourcePool 也已新增**（`-collector.resourcepool`，
> 默认启用）。设计与取舍见 `docs/DESIGN-resourcepool-vsan.md`。
>
> **Network（端口组）与 DVS 仍未采集**，且是主动决定不做而非遗漏 ——
> 理由记在设计稿开头的范围界定里。本文其余部分未随之改写，
> 读的时候请把 vSAN / ResourcePool 两项视为「已补齐」。
