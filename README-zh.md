...
| `-scrape.perf-interval-cache-ttl` | duration | `/metrics` 路径上「按实体类型协商采样间隔」所用 `QueryPerfProviderSummary` 结果的复用有效期。host、vm、datastore 每轮各协商一次，而 govmomi 的 `Manager.ProviderSummary` 名义按 entity.Type 缓存、实际每次都发 SOAP（且 Manager 每登录新建），不缓存时每轮固定多花约 3 次跨网络往返。缓存键含 target、vCenter About version/build 与实体类型；协商与 ESXi 纠偏逻辑仍每轮实时跑，只缓存那次 SOAP。设为 `0` 每次抓取实时协商。与其他缓存同为进程级、**绝不用于多租户 `/probe` 路径**。vcsim 实测单轮约 5.0 ms / 606 KiB / 5723 次分配降到约 3.3 ms / 391 KiB / 4474 次分配（见 `docs/perf/p-provider-summary-cache.txt`）。 | `10m` |
...
| `-vmware.deny-private-addresses` | bool | exporter 主动连 vCenter 这一侧的 SSRF 加严开关（默认 **false**）。无论是否开启，拨号器在 `connect(2)` 前都会按本次**实际解析出的 IP** 拒绝链路本地（含云元数据端点 `169.254.169.254`）与未指定地址（`0.0.0.0`/`::`），校验与连接共用同一次解析，DNS rebinding 无 TOCTOU 窗口。设为 `true` 会**额外**拒绝环回与 RFC1918/IPv6 ULA 私网——仅当 exporter 经可路由地址访问 vCenter 时才开启；vCenter 部署在局域网内（最常见）或经 `127.0.0.1` sidecar 代理时务必保持 false，否则连不上。支持 SIGHUP 热重载。 | `false` |
| `-probe.deny-query-credentials` | bool | `/probe` 凭证收敛开关（默认 **false**，兼容旧的 `?username=&password=` GET 抓取方式）。开启后，凡 URL **查询串**里带 `username`/`password` 的请求一律 400，强制凭证只走 POST 表单体或 HTTP Basic Auth，从而不进 exporter/反代访问日志、`Referer` 头、浏览器历史与链路追踪。判定只看查询串、与 HTTP 方法无关，所以「POST 但凭证仍写在 URL 里」也会被挡。支持 SIGHUP 热重载。 | `false` |
...
| `-log.level` | string | 日志级别: `debug`, `info`, `warn`, `error`。支持 SIGHUP 热重载。 | `info` |
