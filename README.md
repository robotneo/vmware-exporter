...
| -probe.allowed-targets | Comma-separated allowlist for the `/probe` target host, as a defence-in-depth against SSRF: suffixes starting with `.` (`.example.com`), CIDRs containing `/` (`10.0.0.0/8`), or exact host/IP matches. Empty (default) allows any target |
...
| -vmware.perf.chunk-size | Maximum number of entities (hosts/VMs/datastores) carried in one QueryPerf SOAP request (default 64). Large estates otherwise exceed vCenter's `vpxd.stats.maxQueryMetrics` limit or hit the single-request timeout. Chunks run with bounded concurrency (`-collector.max-concurrency`) and merge back in entity order, so emitted series are identical to an unchunked request. Set 0 to send every entity in one request (the pre-v0.1.20 behaviour) |
| -vmware.perf.chunk-size | Maximum number of entities (hosts/VMs/datastores) carried in one QueryPerf SOAP request (default 64). Large estates otherwise exceed vCenter's `vpxd.stats.maxQueryMetrics` limit or hit the single-request timeout. Chunks run with bounded concurrency (`-collector.max-concurrency`) and merge back in entity order, so emitted series are identical to an unchunked request. Set 0 to send every entity in one request (the pre-v0.1.20 behaviour) |
